...
document.querySelector('.newRoom').addEventListener('click', () =>{
    ...
    const itemTypes = types.reduce((text, type,index)=> { return ` ${text} <option value="${index}">${type}</option>`},'')
    const itemDimensions = dimensions.reduce((text, type,index)=> { return ` ${text} <option value="${index}">${type}</option>`},'')
    const itemStyles = styles.reduce((text, type,index)=> { return ` ${text} <span >${type}</span>`},'')
    newRoom.innerHTML = `
    <div class="room-header">
        <p class="room-name">Room1</p>
